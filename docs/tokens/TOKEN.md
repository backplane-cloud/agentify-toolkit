...
# tool prompt
agentify run agent.yaml --toolprompt # Explicit tool prompt for approval to call tool

# Idea, list all providers with a count of total input/output tokens
agentify provider usage list
agentify provider usage show agent.yaml
```

> Notes: The tool schemas bloat the context window which costs $$$
